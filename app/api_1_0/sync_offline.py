...
def sync_source():
    try:
        ...
        if sync_record:
            ...
            sync_record = SyncRecords(system_name=SOURCE_PG_DB_SERVER_IP,
        ...
        try:
            ...
            uuids_and_troop_in_offline = dbsession.query(OfflineCustomer).with_entities(OfflineCustomer.uuid,
                                                                                        OfflineCustomer.troop_number,
                                                                                        OfflineCustomer.username).filter(
                OfflineCustomer.valid == 1,
                or_(OfflineCustomer.create_time > sync_time, OfflineCustomer.update_time > sync_time)).all()
            ...
            uuids_and_troop_in_online = target_dbsession.query(Customer).with_entities(Customer.uuid,
                                                                                       Customer.troop_number,
            ...
            uuids_and_troop_in_offline_for_dict = dbsession.query(OfflineCustomer).with_entities(OfflineCustomer.uuid,
                                                                                                 OfflineCustomer.troop_number,
                                                                                                 OfflineCustomer.username).filter_by(
                valid=1).all()
            ...
            for offline_customer in uuids_and_troop_in_offline_for_dict:
                ...
                customer_uuid_dict_trans[offline_customer[0]] = online_dict_trans[
                    offline_customer[1] + ':' + offline_customer[2]] if online_dict_trans.get(
                    offline_customer[1] + ':' + offline_customer[2], "") else offline_customer[0]

            # offline-online：计算是否有要插入的数据
            offline_troop_numbers_to_insert = list(
                set(customer_diff_in_offline).difference(set(customer_diff_in_online)))
            ...
            offline_troop_numbers_to_update = list(
                set(customer_diff_in_offline).intersection(set(customer_diff_in_online)))
            ...
            if offline_troop_numbers_to_insert:
                offline_customers = dbsession.query(OfflineCustomer).filter(
                    OfflineCustomer.troop_number.in_(troop_numbers_to_insert),
                    OfflineCustomer.username.in_(username_to_insert)).all()
                ...
                if offline_troop_numbers_to_update:
                    offline_customers = dbsession.query(OfflineCustomer).filter(
                        OfflineCustomer.troop_number.in_(troop_numbers_to_update),
                        OfflineCustomer.username.in_(username_to_update)).all()
                    for offline_customer in offline_customers:
                        online_customer = target_dbsession.query(Customer).filter_by(uuid=offline_customer.uuid,
                                                                                     _source=offline_customer._source,
                                                                                     valid=1).first()
                        if online_customer:
            ...
            res_records.append(
                f"用户表同步完成！新增{len(offline_troop_numbers_to_insert)}条数据，更新{len(offline_troop_numbers_to_update)}条数据。")
        except Exception as e:
            print(str(e))
        ...
        try:
            ...
            uuids_and_names_in_offline_ec = dbsession.query(OfflineEntityCategory).with_entities(
                OfflineEntityCategory.uuid,
            ...
            # 遍历offline所有数据，不做时间筛选，构造dict
            uuids_and_names_in_offline_ec_for_dict = dbsession.query(OfflineEntityCategory).with_entities(
                OfflineEntityCategory.uuid,
                OfflineEntityCategory.name).filter(
        ...
        try:
            ...
            relation_categories_in_offline = dbsession.query(OfflineRelationCategory).filter(
                ...
                or_(OfflineRelationCategory.create_time > sync_time,
                    OfflineRelationCategory.update_time > sync_time)).all()
            ...
            for offline_rc in relation_categories_in_offline:
                ...
                # 查找相同关系名称的数据
                relation_same = target_dbsession.query(RelationCategory).filter_by(
                    relation_name=offline_rc.relation_name, valid=1).all()
                if relation_same:
        ...
        try:
            ...
            names_and_cate_uuids_in_online = target_dbsession.query(Entity).with_entities(Entity.name,
                                                                                          Entity.category_uuid,
                                                                                          Entity.uuid).filter(
                Entity.valid == 1, Entity.category_uuid != None).all()
            ...
            for offline_entity in names_and_cate_uuids_in_offline:
                ...
                entity_uuid_dict_trans[offline_entity[2]] = online_dict_trans_entity[
                    offline_entity[0] + str(offline_entity[1])] if online_dict_trans_entity.get(
                    offline_entity[0] + str(offline_entity[1]),
            ...
            if offline_inter:
                online_entities_update = target_dbsession.query(Entity).filter(Entity.name.in_(offline_name_inter),
                                                                               Entity.category_uuid.in_(
                                                                                   offline_cate_uuid_inter)).all()
        ...
        try:
            ...
            # online所有name，唯一性（去重）判断
            uuids_and_names_in_online_evcl = target_dbsession.query(EventClass).with_entities(EventClass.uuid,
                                                                                              EventClass.name).filter(
            ...
            if offline_uuids_to_update:
                ...
                for offline_evcl in offline_event_classes:
                    online_evcl = target_dbsession.query(EventClass).filter_by(uuid=offline_evcl.uuid,
                                                                               _source=offline_evcl._source,
                                                                               valid=1).first()
        ...
        try:
            ...
            # 更新event_class_uuid
            event_categories_in_offline = dbsession.query(OfflineEventCategory).filter(OfflineEventCategory.valid == 1,
                                                                                       or_(
                                                                                           OfflineEventCategory.create_time > sync_time,
                                                                                           OfflineEventCategory.update_time > sync_time)).all()
            ...
            if offline_uuids_to_update:
                ...
                for offline_event_cate in offline_event_categories:
                    online_event_cate = target_dbsession.query(EventCategory).filter_by(uuid=offline_event_cate.uuid,
                                                                                        _source=offline_event_cate._source,
                                                                                        valid=1).first()
        ...
        try:
            ...
            doc_mark_comment_in_offline = dbsession.query(OfflineDocMarkComment).filter(
                OfflineDocMarkComment.valid == 1,
                or_(
            ...
            doc_mark_comment_uuids_in_online = target_dbsession.query(DocMarkComment).with_entities(
                DocMarkComment.uuid).filter_by(valid=1).all()
            ...
            # offline-online：计算是否有要插入的数据
            offline_dmm_to_insert = list(
                set(doc_mark_comment_uuids_in_offline).difference(set(doc_mark_comment_uuids_in_online)))
            # offline-online：计算是否有要更新的数据
            offline_dmm_to_update = list(
                set(doc_mark_comment_uuids_in_offline).intersection(set(doc_mark_comment_uuids_in_online)))

            # 同步offline_doc_mark_comment
            if offline_dmm_to_insert:
                offline_dmm_to_insert = dbsession.query(OfflineDocMarkComment).filter(
                    OfflineDocMarkComment.uuid.in_(offline_dmm_to_insert)).all()
                sync_doc_mark_comments = [
                    DocMarkComment(uuid=i.uuid, doc_uuid=i.doc_uuid, name=i.name, position=i.position,
            ...
            if offline_dmm_to_update:
                for dmm_uuid_offline in offline_dmm_to_update:
                    dmm_online = target_dbsession.query(DocMarkComment).filter_by(uuid=dmm_uuid_offline,
                                                                                  valid=1).first()
                    dmm_offline = dbsession.query(OfflineDocMarkComment).filter_by(uuid=dmm_uuid_offline,
                                                                                   valid=1).first()
        ...
        def insert_and_update_records(source_db, target_db, fields=[]):
            ...
            uuids_in_target_db = target_dbsession.query(target_db).with_entities(target_db.uuid).filter_by(
                valid=1).all()
        ...
        try:
            ...
            fields = ["uuid", "doc_uuid", "word", "entity_uuid", "source", "create_by_uuid", "create_time",
                      "update_by_uuid", "update_time", "appear_text", "appear_index_in_text", "_source", "valid",
                      "position"]
        ...
        try:
            ...
            for place_uuid in place_uuid_list:
        ...
        try:
            ...
            for i in doc_mark_relation_property_in_offline:
                if rc_uuid_dict_trans.get(i.relation_uuid):  # 新插入的doc_rc的外键relation_uuid在本次同步数据内
                    ...
                    rc_online = target_dbsession.query(RelationCategory).filter(
                        ...
                        RelationCategory.source_entity_category_uuids.op('@>')(
                            offline_rc_old.source_entity_category_uuids),
                        RelationCategory.target_entity_category_uuids.op('@>')(
                            offline_rc_old.target_entity_category_uuids),
            ...
            fields = ["uuid", "doc_uuid", "nid", "relation_uuid", "relation_name", "start_time", "start_type",
                      "end_time",
            ...
            insert_num, update_num = insert_and_update_records(OfflineDocMarkRelationProperty, DocMarkRelationProperty,
                                                               fields)
        ...
        try:
            ...
            doc_mark_time_tag_in_offline = dbsession.query(OfflineDocMarkTimeTag).filter(
                OfflineDocMarkTimeTag.valid == 1,
                or_(
        ...
        try:
            ...
            fields = ["uuid", "name", "parent_uuid", "doc_uuid", "create_time", "update_time", "_source", "valid",
                      "position"]
        ...
        try:
            ...
            uuids_in_online = target_dbsession.query(DocMarkAdvise).with_entities(DocMarkAdvise.uuid).filter_by(
                valid=1).all()
            ...
            if doc_mark_advise_to_insert:
                doc_mark_advise_to_insert = dbsession.query(OfflineDocMarkAdvise).filter(
                    OfflineDocMarkAdvise.uuid.in_(doc_mark_advise_to_insert)).all()
                sync_doc_mark_advises = [
                    DocMarkAdvise(uuid=i.uuid, doc_uuid=i.doc_uuid, mark_uuid=i.mark_uuid, type=i.type,
                                  content=i.content,
            ...
            if doc_mark_advise_to_update:
                for dma_uuid_offline in doc_mark_advise_to_update:
                    doc_mark_advise = target_dbsession.query(DocMarkAdvise).filter_by(uuid=dma_uuid_offline,
                                                                                      valid=1).first()
                    dma_offline = dbsession.query(OfflineDocMarkAdvise).filter_by(uuid=dma_uuid_offline,
                                                                                  valid=1).first()
        ...
        try:
            ...
            fields = ["uuid", "event_id", "event_desc", "event_subject", "event_predicate", "event_object",
                      "event_time",
            ...
            for offline_item in offline_catalog_path:
                ...
                if offline_item[1] in online_catalog_path_dict.keys() and offline_item[0] in online_catalog_path_dict[
                    ...
                    offline_online_catalog_dict[offline_catalog_uuids[offline_index]] = online_catalog_uuids[
                        online_index]  # 存进字典
                    tmp_online_catalog = target_dbsession.query(Catalog).filter_by(
                        uuid=online_catalog_uuids[online_index]).first()
