...
from sqlalchemy import or_, and_, extract, func, text, desc
...
def get_schedules_according_to_remind_time():
    try:
        ...
        schedules = Schedule.query.filter(or_(Schedule.remind_time >= cur_time, Schedule.start_time > cur_time),
                                          Schedule.valid == 1, Schedule.customer_uuid == customer_uuid).order_by(Schedule.start_time).all()
...
def get_schedule_by_customer_uuid():
    try:
        customer_uuid = request.args.get('customer_uuid', None)
        schedule = Schedule.query.filter_by(customer_uuid=customer_uuid, valid=1).order_by(Schedule.start_time).all()
        sche_dict = {}
        dict_in_result = {}
        final_result = []
        for i in schedule:
            ...
            if not sche_dict.get(cur_start_time):
                ...
                dict_in_result["month"] = month
                dict_in_result["day"] = day
                print("schedules:", sche_dict[cur_start_time])
                dict_in_result["schedule"] = sche_dict[cur_start_time]
            else:
                temp = {
                    "uuid": i.uuid,
