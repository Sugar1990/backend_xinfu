...
from .. import db
from ..conf import ES_SERVER_IP, ES_SERVER_PORT, PLACE_BASE_NAME, LOCAL_IP
...
# 按id的list查询
@blue_print.route('/get_doc_mark_event_by_ids', methods=['POST'])
def get_doc_mark_event_by_ids():
    try:
        uuids = request.json.get("uuids", [])
        doc_mark_events = db.session.query(DocMarkEvent).filter(
            DocMarkEvent.uuid.in_(uuids), DocMarkEvent.valid == 1).all()
...
def add_doc_mark_event():
    try:
        ...
        if event_address and isinstance(event_address, list):
            ...
            if place_ids:
                ...
                if places:
                    ...
                    if event_object or event_subject:
                        ...
                        if event_object and isinstance(event_object, list):
                            object_ids = DocMarkEntity.query.with_entities(DocMarkEntity.entity_uuid).filter(
                                DocMarkEntity.uuid.in_(event_object), DocMarkEntity.valid == 1).all()
                        if event_subject and isinstance(event_subject, list):
                            subject_ids = DocMarkEntity.query.with_entities(DocMarkEntity.entity_uuid).filter(
                                DocMarkEntity.uuid.in_(event_subject), DocMarkEntity.valid == 1).all()
                        if object_ids or subject_ids:
                            ...
                            if objects or subjects:
                                object_insert_ids, subject_insert_ids = [], []
                                for object in objects:
                                    doc_mark_entity = DocMarkEntity.query.filter_by(entity_uuid=object.uuid,
                                                                                    doc_uuid=doc_uuid,
                                                                                    valid=1).first()
                                    if doc_mark_entity:
                                ...
                                if event_time and isinstance(event_time, list):
                                    ...
                                    if times:
                                        ...
                                        doc_mark_event = DocMarkEvent(uuid=uuid.uuid1(), event_id=event_id,
                                                                      ...
                                                                      add_time=add_time, position=position,
                                                                      valid=1)
...
def modify_doc_mark_event():
    try:
        ...
        if doc_mark_event:
            ...
            if position:
                doc_mark_event.position = position
...
def get_advanced_search_of_events():
    try:
        relation_uuid = request.json.get('relation_uuid', None)  # yc专用参数，有值时去ar里搜寻事件关系数据
        dates = request.json.get('dates', {})
        time_tag_ids = []
        location_range = request.json.get('location_range', [])
        ...
        for object in object_list:
            if object.get("category_uuid", "") and object.get("entity", ""):
                ...
                entity_db = Entity.query.filter(Entity.category_uuid == category_uuid,
                                                or_(Entity.name.like(f'%{entity}%'),
        ...
        if not relation_uuid:
            ...
            for doc_mark_event in doc_mark_events:
                ...
                for event_time_uuid in doc_mark_event.event_time:
                    ...
                    if event_time_uuid in time_tag_ids:
                        doc_mark_time_tag = DocMarkTimeTag.query.filter(DocMarkTimeTag.uuid == event_time_uuid,
                ...
                place = []
                doc_mark_place_id_list = doc_mark_event.event_address if doc_mark_event.event_address else []
            ...
            event_list_entity = [sorted(i, key=lambda x: x.get('datetime', '')) for i in event_dict.values() if
                                 len(i) > 1]
            event_list_sorted = sorted(event_list, key=lambda x: x.get('datetime', '')[0])
            print('before', len(event_list_sorted), len(event_list_entity), )
            if location_range:
                event_list_sorted, event_list_entity = judge_place_range(event_list_sorted, event_list_entity,
                                                                         location_range)
                print('after_location',len(event_list_sorted),len(event_list_entity),)
            ...
            for doc_mark_event in doc_mark_events:
                para = {
                    ...
                    "filter_dict": {"relation_uuid": relation_uuid}
                }
                url = "http://{}:{}".format(LOCAL_IP, ES_SERVER_PORT) + '/arango/query_data'
                data = json.dumps(para)
                result = requests.post(url=url, data=data)
                if result.status_code in (200, 201):
                    event_relation_records = json.loads(result.text)
                    for record in event_relation_records:
                        if record.get('source_event_uuid') == doc_mark_event.uuid or record.get(
                                'target_event_uuid') == doc_mark_event.uuid:
                            event_result.append(record)
                    res = success_res(data=event_result)
                else:
...
def judge_place_range(event_list, entity_list, location_range):
    min_long = min(location_range[0][0], location_range[1][0])
    max_long = max(location_range[0][0], location_range[1][0])
    min_lat = min(location_range[0][1], location_range[1][1])
    max_lat = max(location_range[0][1], location_range[1][1])
    out_event_list = []
    for event in event_list:
        place_new = [place_dot for place_dot in event["place"] if place_dot["place_lon"] and place_dot["place_lat"] and
                     float(place_dot["place_lon"]) < max_long and float(place_dot["place_lon"]) > min_long
                     and float(place_dot["place_lat"]) > min_lat and float(place_dot["place_lat"]) < max_lat]
        if place_new:
            event["place"] = place_new
            out_event_list.append(event)
    out_entity_list = []
    for line in entity_list:
        ...
        for event in line:
            ...
            if place_new:
                event["place"] = place_new
                list_of_this_line.append(event)
        if list_of_this_line:
            out_entity_list.append(list_of_this_line)
    return out_event_list, out_entity_list
    ...
    try:
        ...
        for doc_mark_event in doc_mark_events:
            if set(doc_mark_event.event_time) & doc_mark_time_tag_ids_set:
                ...
                datetime = [DocMarkTimeTag.query.filter_by(uuid=time_id, valid=1).first().format_date.strftime(
                    '%Y-%m-%d %H:%M:%S')]
                try:
                    datetime.append(
                        DocMarkTimeTag.query.filter_by(uuid=time_id, valid=1).first().format_date_end.strftime(
                            '%Y-%m-%d %H:%M:%S'))
                except:
                    pass
